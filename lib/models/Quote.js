...
export default class Quote {
  ...
  static async findByName(name) {
    const { rows } = await pool.query(`
    SELECT lclquotes.id, name, quote_text, quote_text_only
    ...
    WHERE quotes_persons_junction.persons_id = (SELECT id FROM persons WHERE persons.name = $1)`, [name])
    return rows.map(row => new Quote(row));
  }
  ...
  static async findByLetter(searchTerm) {
    ...
    SELECT lclquotes.id, name, quote_text, quote_text_only
    FROM lclquotes
    INNER JOIN quotes_persons_junction ON quotes_persons_junction.quotes_id = lclquotes.id
    INNER JOIN persons ON persons.id = quotes_persons_junction.persons_id
